...
def from_snow(schema='postgres'
  ...
  if query != None: qry = query
  else: qry = get_query(kind=q_kind)
  ...
  try:
    ...
    if to_df: results = __list_to_df(data=data,q_kind=q_kind)
    else: results = data
...
def to_snow(df=None
  ...
  try: type(schema)==str
    ...
    if schema==None: print('Schema element not specified.')
    else: print('Schema element is not a string.')
